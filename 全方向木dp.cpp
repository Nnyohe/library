...
struct Rerooting {
  ...
  function<DP(DP)> add_root = [](DP d) -> DP { return DP((d.dp+1) % M); };
  /* end 問題ごとに書き換え */
  // グラフの定義
  struct Edge {
    int to;
  ...
  DP dfs(int v, int p = -1) {  // 頂点v, 親p
    ...
    // addrootで1を足す理由は部分木の根が白である場合(つまり部分木の頂点全てが白色である場合)を考慮している。
  ...
  void bfs(int v, const DP& dp_p,
    ...
    ans[v] = add_root(dp_l[deg]);  // 頂点 v の答え(正確には異なる)
...
int main() {
  ...
  reroot.build();
  for (int i = 0; i < N; i++) {
    //-1する必要があるのは ans[v] = add_root(dp_l[deg]);によって頂点iが白色になるパターン(すなわち全部白色であるパターン)も含まれているため
    cout << (reroot.ans[i].dp-1)%M << endl;
  }
}